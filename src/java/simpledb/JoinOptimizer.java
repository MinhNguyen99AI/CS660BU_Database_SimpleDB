...
public class JoinOptimizer {
    ...
    public Vector<LogicalJoinNode> orderJoins(
    	...
    	Set<Set<LogicalJoinNode>> j = enumerateSubsets(this.joins, 1);
    	PlanCache planCache = new PlanCache();
    	for (int i = 0; i<=j.size(); ++i) {
    		Set<Set<LogicalJoinNode>> subSetOfJ = enumerateSubsets(this.joins,i);
    		for (Set<LogicalJoinNode> s : subSetOfJ) {
    			CostCard bestPlan = new CostCard();
    			bestPlan.plan = null;
    			bestPlan.cost = Double.MAX_VALUE;
    			bestPlan.card = Integer.MAX_VALUE;
    			for (LogicalJoinNode s_prime : s) {
    				 CostCard subPlan = computeCostAndCardOfSubplan(stats, filterSelectivities,s_prime,s,bestPlan.cost, planCache);
                     if (subPlan!=null && subPlan.cost < bestPlan.cost) {
                         bestPlan = subPlan;
                         //bestPlan.cost = subPlan.cost;
                         //bestPlan.card = subPlan.card;
                     }     
    			}
    			planCache.addPlan(s,bestPlan.cost, bestPlan.card, bestPlan.plan);
    		}
    	}
    	Vector<LogicalJoinNode> v = planCache.getOrder(joinSet); 
        return v;
    }
