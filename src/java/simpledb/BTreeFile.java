...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		BTreeInternalPage noneLeafPage = (BTreeInternalPage) getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
		BTreeEntry entry = null;
		Iterator<BTreeEntry> iterate = noneLeafPage.iterator();
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		int maxNumTuple = page.getMaxTuples()/2;
		int tupleIndex = 0;
		Tuple currentTuple = null;
		Iterator<Tuple> iterateTuple = page.reverseIterator();
		while (iterateTuple.hasNext() && tupleIndex < maxNumTuple) {
			currentTuple = iterateTuple.next();
			page.deleteTuple(currentTuple);
			newPage.insertTuple(currentTuple);
			tupleIndex++;
		}
		
		BTreeEntry newEntry = new BTreeEntry(currentTuple.getField(keyField), page.getId(), newPage.getId()); 
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), newEntry.getKey());
		parent.insertEntry(newEntry);
		newPage.setParentId(parent.getId());
		
		if (page.getRightSiblingId() != null) {
			BTreeLeafPage oldPageRightSib = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
			newPage.setRightSiblingId(oldPageRightSib.getId());
			oldPageRightSib.setLeftSiblingId(newPage.getId());
		}
		page.setRightSiblingId(newPage.getId());
		newPage.setLeftSiblingId(page.getId());
		
		
		if(newEntry.getKey().compare(Op.GREATER_THAN, field))
			return page;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int maxNumEntries = page.getMaxEntries()/2;
		int entryIndex = 0;
		BTreeEntry currentEntry = null;
		Iterator<BTreeEntry> iterateEntry = page.reverseIterator();
		while (iterateEntry.hasNext() && entryIndex < maxNumEntries) {
			currentEntry = iterateEntry.next();
		...
		currentEntry = iterateEntry.next();
		page.deleteKeyAndRightChild(currentEntry);
		BTreeInternalPage internalParent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), currentEntry.getKey());
		BTreeEntry newEntry = new BTreeEntry(currentEntry.getKey(), page.getId(), newPage.getId());
		internalParent.insertEntry(newEntry);
		
		newPage.setParentId(internalParent.getId());
		updateParentPointers(tid, dirtypages, newPage);
		
		if(currentEntry.getKey().compare(Op.GREATER_THAN, field)) 
			return page;
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int tupleMoveNum = (sibling.getNumTuples() - page.getNumTuples())/2;
		
		Iterator<Tuple> iterate_Tuple;
		Tuple currentTuple = null;
		if (!isRightSibling) {
			iterate_Tuple = sibling.reverseIterator();
		}
		else {
			iterate_Tuple = sibling.iterator();
		}

		for (int i = 0; i< tupleMoveNum; i++) {
			currentTuple = iterate_Tuple.next();
		...
		entry.setKey(currentTuple.getField(keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int entryMoveNum = (leftSibling.getNumEntries() - page.getNumEntries())/2;
		...
		BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), leftChild, rightChild);
		page.insertEntry(newEntry);
		BTreeEntry currentEntry = null;
		Iterator<BTreeEntry> entryIterate = leftSibling.reverseIterator();
	
		for (int i = 0; i< entryMoveNum-1; i++) {
			currentEntry = entryIterate.next();
			leftSibling.deleteKeyAndRightChild(currentEntry);
			page.insertEntry(currentEntry);
		}
	
		
		currentEntry = entryIterate.next();
		leftSibling.deleteKeyAndRightChild(currentEntry);
		parentEntry.setKey(currentEntry.getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int entryMoveNum = (rightSibling.getNumEntries() - page.getNumEntries())/2;
		...
		page.insertEntry(newEntry);
		BTreeEntry currentEntry = null;
		Iterator<BTreeEntry> entryIterate = rightSibling.iterator();
		for (int i = 0; i< entryMoveNum-1; i++) {
			currentEntry = entryIterate.next();
			rightSibling.deleteKeyAndLeftChild(currentEntry);
			page.insertEntry(currentEntry);
		}
		currentEntry = entryIterate.next();
		rightSibling.deleteKeyAndLeftChild(currentEntry);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<Tuple> iterate = rightPage.iterator();
		Tuple currentTuple = null;
		while(iterate.hasNext()) {
			currentTuple = iterate.next();
			rightPage.deleteTuple(currentTuple);
			leftPage.insertTuple(currentTuple);
			}
		
		BTreePageId pointerRight = rightPage.getRightSiblingId();
		leftPage.setRightSiblingId(pointerRight);
		if (pointerRight != null) {
			BTreeLeafPage rightLeaf = (BTreeLeafPage) getPage(tid, dirtypages, pointerRight, Permissions.READ_WRITE);
			rightLeaf.setLeftSiblingId(leftPage.getId());
		}
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> iterate = rightPage.iterator();
		if (iterate.hasNext()) {
			BTreeEntry nextEntry = iterate.next();
			BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), lastEntry.getRightChild(), nextEntry.getLeftChild());
